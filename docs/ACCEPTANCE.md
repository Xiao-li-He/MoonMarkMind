...
- `app/web/heading_parser.mbt`：Markdown 标题、列表和结构线索解析。
- `app/web/outline_tree.mbt`：把扁平解析结果组织为层级树。
- `app/web/outline_model.mbt`：维护脑图模型，提供节点增删改移、折叠、隐藏、缩进、提升和 Markdown 回写。
- `app/web/markdown_render.mbt`：处理节点正文中的 Markdown 富文本渲染。
- `lib/latex_math_render`：处理 LaTeX 公式渲染。
- `app/web/sample_data.mbt`：集中维护内置示例。
- `app/web/web_layout.mbt`：生成页面结构和工具栏。
- `app/web/web_dom.mbt`：封装 DOM 读取、写入、下载、导出、编辑器联动等操作。
- `app/web/web_frontend.mbt`：组织前端状态、事件绑定和视图刷新。
- `app/web/web_mindmap_bindings.mbt`：处理脑图视图、缩放、拖拽和节点定位。
- `app/web/web_theme.mbt`：集中管理界面与脑图主题样式。
